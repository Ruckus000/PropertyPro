...
- **Migrations are applied to prod MANUALLY** (Supabase MCP `apply_migration`, then verify via `information_schema`).
  - *CI/CD status:* `deploy.yml` *does* contain a `db:migrate` gate (before the Vercel build; needs the `production`-env `DIRECT_URL` secret, which IS set), added by #683 to prevent drift — but it **fails on every run** (`drizzle-kit migrate` → `check_for_column_name_collision`) because manual out-of-band application left drizzle's `__drizzle_migrations` ledger out of sync, so the step re-applies existing DDL and collides.
  - *Deployment path:* a failed migrate step **skips the deploy**, so `deploy.yml` is *not* the deploy path — prod code ships via Vercel's **native GitHub integration**.
  - *Action required:* do not rely on the pipeline for migrations; apply manually. The drizzle ledger must be re-reconciled (mark 0017/0018/0019 applied with correct hash + `when`) before the auto-migrate gate can safely go green.
  - *Verify, don't assume:* check the pipeline's real behavior via GH Actions **run history**, not just the workflow file — the file alone looks like auto-migrate works.
