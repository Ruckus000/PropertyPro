...
export const RLS_GLOBAL_TABLE_EXCLUSIONS = [
  ...
  { tableName: 'demo_instances', reason: 'Platform demo-provisioning ledger — references a seeded_community_id but is not scoped by it (a row can outlive the community it seeded). RLS is enabled with ZERO policies, which denies every non-privileged caller outright; access is service-role only. Enabled without FORCE, unlike the tenant tables.' },
  {
    tableName: 'cron_runs',
    reason:
      'One row per SCHEDULED JOB (0067) — the heartbeat that lets /api/v1/internal/cron-health detect a cron which stopped running, which failure alerting structurally cannot (a 401ing cron throws AppError and never reaches Sentry). Deliberately NOT tenant-scoped: a job is platform-wide and has no community_id at all, so there is no scoping column to key a policy on. Locked down on the platform-table pattern: RLS enabled AND forced with zero policies (the deny-everyone default), so only the privileged service-role connection — which short-circuits via pp_rls_is_privileged() — can read or write it. Written by withCronJob, read by the unauthenticated health probe, which returns slugs and timestamps only.',
  },
  ...
  },
  {
    tableName: 'cron_runs',
    reason:
      'Cron heartbeat ledger (0067) — one upserted row per scheduled job, seventeen rows forever, keyed on job_slug. Platform-level by construction: a cron tick belongs to the schedule, not to a community, and several of the jobs it tracks scan cross-tenant, so there is no community_id to scope on. RLS is enabled and forced with zero policies, which is the deny-everyone default and closes it to anon and authenticated today. GRANTS DIVERGE FROM EVERY SIBLING, and the divergence is recorded here rather than smoothed over: measured in production 2026-09-06, anon still holds SELECT and authenticated still holds SELECT/INSERT/UPDATE/DELETE, where revenue_snapshots (0035) and user_search_index (0037) both pair the RLS lockdown with REVOKE ALL. 0067 reasoned only about RLS ("with RLS enabled and no policy, non-privileged roles see nothing — which is exactly right"), which is true and is the same reasoning gap this list already records for users, pending_signups and user_search_index: it leaves the table one permissive policy away from letting any logged-in resident DELETE the monitoring signal. A follow-up migration should REVOKE ALL FROM anon, authenticated; until it lands, that is the accepted exposure. Written only by recordHeartbeat in apps/web/src/lib/services/cron-run-service.ts over the unscoped client, and read only by /api/v1/internal/cron-health.',
  },
