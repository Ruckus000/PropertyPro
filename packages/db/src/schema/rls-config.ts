...
export const RLS_GLOBAL_TABLE_EXCLUSIONS = [
  ...
  },
  {
    tableName: 'cron_runs',
    reason:
      'Cron heartbeat ledger (0067) — one upserted row per scheduled job, seventeen rows forever, keyed on job_slug. Platform-level by construction: a cron tick belongs to the schedule, not to a community, and several of the jobs it tracks scan cross-tenant, so there is no community_id to scope on. Posture matches every sibling platform table: RLS enabled and forced with zero policies (the deny-everyone default), plus REVOKE ALL FROM anon, authenticated with service_role retaining CRUD. Verified against production 2026-09-07 — has_table_privilege returns false for anon and authenticated on SELECT/INSERT/UPDATE/DELETE. The lockdown took two migrations, and the gap between them is the reusable lesson: 0067 reasoned only about RLS ("with RLS enabled and no policy, non-privileged roles see nothing — which is exactly right"), which is true and still left Supabase\'s open grant baseline in place, so anon held SELECT and authenticated held full CRUD until 0069 revoked them. That is the same reasoning gap this list already records for users, pending_signups and user_search_index: "not tenant-scoped" read as "needs no further lockdown". Regression-tested by LOCKED_DOWN_TABLES in packages/db/__tests__/rls-policies.integration.test.ts. Written only by recordCronRun/registerCronJobs in apps/web/src/lib/services/cron-run-service.ts over the unscoped client, and read only by /api/v1/internal/cron-health.',
  },
  {
    tableName: 'platform_admin_preferences',
    reason:
      "Per-platform-admin console preferences (0073) — the notification tray's read watermark, the operator's alert opt-ins, and the fingerprints of alerts already pushed to them. No community_id, and that is the premise rather than an omission: a platform admin holds no community membership, which is the whole point of the role, so there is no tenant to scope by. Keyed on user_id ITSELF with no surrogate id and no sequence — one row per operator or none — which is what makes the console's write a plain upsert and why, alone in this family, it needs no sequence revoke. Locked down by 0073 on the 0072 posture: RLS enabled and forced, zero policies (the deny-everyone default), REVOKE ALL from anon/authenticated, service_role retaining CRUD. The only reader and writer is apps/admin over service_role behind requirePlatformAdmin. The anon key ships in the browser bundle, and these rows map a named operator to what they watch and what they have already been told, so the REVOKE is not ceremony. Two CHECK constraints pin the jsonb shapes because NOT NULL does not: a jsonb null is a VALUE, so 'null'/'[]'/'3' all satisfy NOT NULL on alert_prefs.",
  },
  {
    tableName: 'platform_admin_push_subscriptions',
    reason:
      'Web Push subscriptions for the admin console PWA (0073). One row per BROWSER rather than per admin — the Push API mints a subscription per service-worker registration, so a laptop and a phone are legitimately two rows — with endpoint carrying the UNIQUE constraint as the natural key so a re-subscribe updates instead of duplicating deliveries. No community_id for the same reason as platform_admin_preferences. Same 0073 lockdown, plus the SEQUENCE revoke that table does not need: endpoint, p256dh and auth together are enough for anyone holding them to deliver a notification to that operator\'s device, and leaving the bigserial sequence open would make the table look locked down to a privilege check while an INSERT path stayed reachable — the trap 0053 recorded. A CHECK bounds endpoint to https://, which is a bound on where the delivery path will POST, not a format nicety.',
  },
