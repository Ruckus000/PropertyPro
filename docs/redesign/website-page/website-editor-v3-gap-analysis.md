...
| **2** | **2a:** split the four data-fetching renderers (Announcements, Documents, Meetings, Contact) into async shell + pure view — no behaviour change, pure refactor. **2b:** canvas renders the views, section selection, float controls, `Alt+↑/↓`, between-section inserter, drag reorder with keyboard parity, inspector | 1 | the actual redesign | Medium — 2a touches the live public site, so it lands and is verified before 2b |
...
| 3 | **Canvas: reuse `components/public-site/blocks/*`, via a container/presentational split** | Selection chrome wraps *around* each block, as v3's `wrapSection` does. **Corrected 2026-07-25:** only six of the ten renderers (Hero, Text, Image, Faq, Gallery, Amenities) are pure and prop-driven and can be reused as-is. The other four (Announcements, Documents, Meetings, Contact) are **async server components that query the DB inside themselves** via `getPublicCommunityScopedReader`, so they cannot render in a client canvas. Each of those four splits into an async data shell (public site) and a pure view component (shared with the canvas) — one presentational component per block type, so nothing forks. The line falls exactly on v3's own `AUTHORED` / `DATA_DRIVEN` split in `v3-kit.jsx`. Standing constraint after the split: view components stay hook-free and prop-driven — worth a comment in `blocks/registry.ts` so a future edit doesn't silently break the canvas |
