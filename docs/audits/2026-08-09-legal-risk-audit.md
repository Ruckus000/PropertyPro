...
| **Payments** (F-15, F-16) | PaymentIntents are **direct charges** (`{ stripeAccount }`); `transfer_data` is gone and the cut stays `application_fee_amount`. Every later touch — retrieve, update, and the webhook re-reads — passes the connected account. *(Corrected 2026-09-24: this row originally said "three webhook re-reads" and was wrong on both counts. There are seven, across the succeeded/refunded/dispute handlers, and the `latest_charge` lookup in `handlePaymentIntentSucceeded` did not pass the account until 2026-09-24. It is now pinned by the "reads the latest charge from the connected account" test in `apps/web/__tests__/finance/finance-service-webhook.test.ts`.)* Stripe.js loads per-account in the browser. `owner_pays` is **retired**. |
