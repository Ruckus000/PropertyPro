...
Evidence notes: PAG-01 — `grep -rh "CREATE INDEX" packages/db/migrations/*.sql | wc -l` → 76; filtered to keyset tables → **0**. That count was measured at `1566d03e3` and is a snapshot: it moves the moment `0076` lands on `main`. PAG-01 implemented 2026-09-23 as `packages/db/migrations/0076_keyset_indexes_hard_tier.sql`, seven partial indexes, each on `deleted_at IS NULL` because that is exactly what `createScopedClient` injects beside `community_id = ?`. One correction to the plan's own framing: only TWO of the seven DDLs came from the B3 doc verbatim (`faqs` :193-195, `announcements` :213-220); the other five are derived from the SHIPPED `orderBy` of the service that reads each table, since the doc names those only in prose and predates the implementations. Two column lists would have been wrong if written from the doc: `announcements` orders by `published_at`, not `created_at`, and `amenities` has no `is_active` column at all, so its key is `(community_id, name ASC, id ASC)` — two components, not three. Each index was verified causal on a migrated local database: with it, the shipped query plans to an index scan with NO sort node; drop that one index and the same query degrades to a sort; restore it and the sort disappears. Merely creating an index proves nothing — a wrong column list leaves Postgres never using it while every check stays green. PAG-09 cross-refs CON-05 on announcements and SVC-01 on the codecs. PAG-07's order `(opensAt desc, id desc)` is already cursor-ready and `idx_elections_community_dates` already exists.
