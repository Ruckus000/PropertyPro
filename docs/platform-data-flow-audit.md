...
| ~~M-03~~ | **P2** | ~~**Legacy `x-tenant-id` header fallback.**~~ **FIXED 2026-09-07.** `COMMUNITY_ID_HEADERS` is now `['x-community-id']` alone. The test that asserted the fallback worked is inverted rather than deleted, so it now pins the removal. |
