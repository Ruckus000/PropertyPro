...
| ~~#951 Sentry may buffer raw Stripe webhook bodies~~ | 27d | **Fixed, NOT closed.** `scrubServerEvent` drops `request.data` (plus URLs, and headers case-insensitively) on all four server/edge configs, wired to **both** `beforeSend` and `beforeSendTransaction` — the first attempt wired only `beforeSend`, which fires for error events alone, leaving transactions carrying the same body at `tracesSampleRate` 0.1. Closing it needs the issue's own empirical check — throw in the webhook handler on a preview deploy, inspect the real event — which needs access this session did not have |
