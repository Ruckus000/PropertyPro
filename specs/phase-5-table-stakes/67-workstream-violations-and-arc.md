...
### Fine Payment (WS 66 Integration Point)

When a fine is imposed via `POST /api/v1/violations/:id/fine`:
1. A `violation_fines` row is created with status `pending`
2. A ledger entry is posted via `postLedgerEntry()` with `sourceType: 'violation'`
3. An `assessment_line_item` is created in WS 66's table with:
   - `assessmentId`: NULL (fines are not recurring assessments)
   - `unitId`: the violation's unit
   - `amountCents`: the fine amount
   - `dueDate`: fine issuance date + grace period
   - `status`: 'pending'
4. The resident pays the fine through WS 66's standard payment flow
5. On payment completion, WS 66 updates `violation_fines.status` to `'paid'`

This requires WS 66 to support one-off line items (not tied to a recurring assessment). Verify `assessment_line_items.assessmentId` is nullable.

---

## 5. Data Model And Migrations

### New Tables (migrations 0056-0065 range)
...
### Violation Categories (extensible per community)

Initial values: `noise`, `parking`, `pets`, `property_damage`, `unauthorized_modification`, `landscaping`, `common_area_misuse`, `other`

Stored as TEXT, not pgEnum — communities may define custom categories in the future.

### ARC Project Types

Initial values: `exterior_modification`, `structural`, `landscaping`, `fencing`, `painting`, `roofing`, `other`
...
| site_manager | yes | yes | yes | yes | yes |
...
| cam | no | yes | — | yes |
| site_manager | no | yes | — | yes |
| property_manager_admin | no | yes | — | yes |

Board members who are also unit owners submit ARC applications using their `owner` role permissions. The system checks the user's highest-privilege role for review/decide actions, but `owner`-level permissions apply for submissions. This is consistent with the existing role resolution pattern in the codebase.
...
- Concurrent fine imposition on same violation → Use `SELECT ... FROM violations WHERE id = $1 AND status IN ('noticed', 'hearing_scheduled') FOR UPDATE` before inserting a fine. This prevents concurrent fine imposition on the same violation while allowing multiple sequential fines (escalating penalties for repeat offenses).
