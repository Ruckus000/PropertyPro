...
import { MeetingsBlockForm } from '@/components/pm/site-editor/MeetingsBlockForm';
...
describe('<MeetingsBlockForm>', () => {
  it('renders limit and timeWindowDays fields with defaults (10, 30)', () => {
    render(wrap(<MeetingsBlockForm communityId={42} blockOrder={6} initial={null} />));
    expect(screen.getByLabelText(/maximum items/i)).toHaveValue(10);
  ...
  it('submits PATCH with the parsed payload on Save click', async () => {
    ...
    const limitInput = screen.getByLabelText(/maximum items/i);
    await userEvent.clear(limitInput);
    await userEvent.type(limitInput, '7');
    ...
    expect(body.blockType).toBe('meetings');
    expect(body.blockOrder).toBe(6);
    expect(body.content.limit).toBe(7);
  ...
  it('surfaces server error as inline alert', async () => {
    (global.fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      ok: false,
      status: 422,
      json: async () => ({ error: { code: 'VALIDATION_ERROR', message: 'Invalid time window.' } }),
    });
    render(wrap(<MeetingsBlockForm communityId={42} blockOrder={6} initial={null} />));
    fireEvent.click(screen.getByRole('button', { name: /save/i }));
    await waitFor(() => {
      expect(screen.getByRole('alert')).toHaveTextContent('Invalid time window.');
    });
  });
});
