/**
 * Route unit tests — POST /api/v1/admin/join-requests/[id]/deny.
 *
 * Added alongside Plan A1 drain #83. Mirrors drain #81 (approve sibling)
 * test suite exactly. Covers the contracted `runRoute` envelope with
 * **header-only communityId** resolution and **empty-body tolerance**
 * (mirrors drain #72 esign/cancel). Audit log preserved post-service-call
 * (drain #73 precedent).
 *
 * Test cases:
...
vi.mock('@/lib/join-requests/approve-request', () => ({
  denyJoinRequest: denyJoinRequestMock,
...
import { POST } from '../../src/app/api/v1/admin/join-requests/[id]/deny/route';
...
const SERVICE_RESULT = {
  requestId: 7,
  deniedAt: new Date('2026-05-27T00:00:00.000Z'),
};
...
describe('POST /api/v1/admin/join-requests/[id]/deny', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    denyJoinRequestMock.mockResolvedValue(SERVICE_RESULT);
    logAuditEventMock.mockResolvedValue(undefined);
  });

  it('denies with notes; service + audit called with notes (happy path)', async () => {
    const res = await POST(
      postReq(7, { body: { notes: 'Not eligible' } }),
      routeCtx('7'),
    );
    ...
    expect(denyJoinRequestMock).toHaveBeenCalledWith({
      requestId: 7,
    ...
    expect(logAuditEventMock).toHaveBeenCalledWith({
      ...
      resourceId: '7',
      metadata: { notes: 'Not eligible' },
    });
  });

  it('denies with no body (empty-body tolerance); audit metadata.notes = null', async () => {
  ...
  it('returns 401 when unauthenticated; downstream gates not called', async () => {
    ...
    expect(denyJoinRequestMock).not.toHaveBeenCalled();
