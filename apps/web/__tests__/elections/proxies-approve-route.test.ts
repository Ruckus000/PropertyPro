/**
 * Route unit tests — `POST /api/v1/elections/[id]/proxies/[proxyId]/approve`.
 *
 * Added alongside Plan A1 drain #47. **First two-param route in the corpus**:
 * tests cover BOTH `params.id` and `params.proxyId` validation
 * independently. Covers the contracted runRoute envelope: happy path,
 * 401 unauth, 400 invalid params.id / invalid params.proxyId /
 * missing-communityId / invalid-communityId, 403 demo-grace, 403 non-member,
 * 403 elections-disabled, 403 permission, 403 admin-role, x-request-id null
 * forwarding.
 */
...
import { POST } from '../../src/app/api/v1/elections/[id]/proxies/[proxyId]/approve/route';
...
function jsonPost(
  ...
  return new NextRequest(
    `http://localhost:3000/api/v1/elections/${id}/proxies/${proxyId}/approve`,
    {
      method: 'POST',
...
describe('POST /api/v1/elections/[id]/proxies/[proxyId]/approve', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    approveElectionProxyForCommunityMock.mockResolvedValue(APPROVED);
  });

  it('approves the proxy and returns wrapped service result (happy path)', async () => {
    ...
    expect(approveElectionProxyForCommunityMock).toHaveBeenCalledWith(
      42,
      7,
      3,
      'user-admin-1',
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    expect(approveElectionProxyForCommunityMock).not.toHaveBeenCalled();
  });
