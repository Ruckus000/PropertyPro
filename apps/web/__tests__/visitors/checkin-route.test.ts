/**
 * Route unit tests — `PATCH /api/v1/visitors/[id]/checkin`.
 *
 * Added alongside Plan A1 drain #53. Covers the contracted runRoute envelope:
...
const {
  ...
  checkInVisitorForCommunityMock,
} = vi.hoisted(() => ({
  ...
  checkInVisitorForCommunityMock: vi.fn(),
}));
...
vi.mock('@/lib/services/package-visitor-service', () => ({
  checkInVisitorForCommunity: checkInVisitorForCommunityMock,
}));

import { PATCH } from '../../src/app/api/v1/visitors/[id]/checkin/route';
...
function jsonPatch(
  ...
  return new NextRequest(
    `http://localhost:3000/api/v1/visitors/${id}/checkin`,
    {
      method: 'PATCH',
...
describe('PATCH /api/v1/visitors/[id]/checkin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    checkInVisitorForCommunityMock.mockResolvedValue(CHECKIN_RESULT);
  });

  it('checks in the visitor (happy path)', async () => {
    ...
    expect(checkInVisitorForCommunityMock).toHaveBeenCalledWith(
      42,
      7,
      'user-staff-1',
      'req-abc',
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    expect(checkInVisitorForCommunityMock).not.toHaveBeenCalled();
  });
