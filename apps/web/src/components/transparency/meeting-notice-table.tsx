        ...
        {meetings.length === 0 ? (
          ...
          <>
                  ...
                  {meetings.map((meeting) => (
                    <tr key={meeting.id}>
                      <td className="py-3 pr-3">
                        ...
                        <p className="text-xs text-gray-500">Starts: {formatDate(meeting.startsAt, timezone)}</p>
                      </td>
            ...
            <div className="space-y-3 sm:hidden">
              {meetings.map((meeting) => (
                <article key={`mobile-${meeting.id}`} className="rounded-md border border-gray-200 p-3">
                  <h3 className="text-sm font-semibold text-gray-900">{meeting.title}</h3>
                  <p className="text-xs uppercase tracking-wide text-gray-500">{meeting.meetingType}</p>
                  <dl className="mt-2 space-y-1 text-sm text-gray-600">
                    <div>
                      <dt className="inline font-medium text-gray-700">Starts: </dt>
                      <dd className="inline">{formatDate(meeting.startsAt, timezone)}</dd>
                    </div>
                    <div>
                      <dt className="inline font-medium text-gray-700">Notice Posted: </dt>
                      <dd className="inline">{meeting.noticePostedAt ? formatDate(meeting.noticePostedAt, timezone) : 'Not recorded'}</dd>
                    </div>
                    <div>
                      <dt className="inline font-medium text-gray-700">Lead Time: </dt>
