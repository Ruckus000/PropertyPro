...
interface Props {
  communityId: number;
  blockOrder: number;
  initial: MeetingsBlockContent | null;
  onSaved?: () => void;
}

export function MeetingsBlockForm({ communityId, blockOrder, initial, onSaved }: Props) {
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault();
    setServerError(null);
    const payload: MeetingsBlockContent = { limit, timeWindowDays };
    try {
      await mutation.mutateAsync({ blockType: 'meetings', blockOrder, content: payload });
      onSaved?.();
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <p className="text-sm text-content-secondary">
        Renders upcoming community meetings on the public site.
      </p>
      <div>
        <label htmlFor={`meet-limit-${blockOrder}`} className="block text-sm font-medium text-content">
          Maximum items <span className="text-danger">*</span>
        </label>
        <input
          id={`meet-limit-${blockOrder}`}
          type="number"
          min={1}
          max={20}
          value={limit}
      ...
      <div>
        <label htmlFor={`meet-window-${blockOrder}`} className="block text-sm font-medium text-content">
          Time window (days) <span className="text-danger">*</span>
        </label>
        <input
          id={`meet-window-${blockOrder}`}
          type="number"
          min={1}
          max={365}
        ...
        <p className="mt-1 text-xs text-content-secondary">
          Only meetings starting within this many days from today will appear.
        </p>
      </div>
      {serverError && (
