    ...
    <Popover modal>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {count > 0 && (
            <span
              className="absolute right-1.5 top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--status-danger)] px-1 text-xs font-semibold leading-none text-white lg:right-0.5 lg:top-0.5"
              aria-live="polite"
              aria-atomic="true"
            >
              {count > 9 ? '9+' : count}
      ...
      <PopoverContent align="end" sideOffset={8} className="w-80 overflow-hidden rounded-[var(--radius-lg)] p-0">
        <NotificationDropdown communityId={communityId} />
      </PopoverContent>
    </Popover>
  );
}
