...
export function parseCommunityIdFromQuery(req: NextRequest): number {
...
export function parseCommunityIdFromBody(req: NextRequest, rawCommunityId: number): number {
  if (!Number.isInteger(rawCommunityId) || rawCommunityId <= 0) {
    throw new BadRequestError('communityId must be a positive integer');
  }

  return resolveEffectiveCommunityId(req, rawCommunityId);
}
