/**
 * Visitors — check in (staff-operator action).
 *
 * PATCH /api/v1/visitors/[id]/checkin
 * Body: { communityId }
 *
 * Plan A1 drain #53. Migrated to `runRoute(contract, handler)`; see
 * `./contract.ts` for the schema and rationale. HTTP method is PATCH
 * (not POST) — visitor check-in is a state mutation on an existing
 ...
 *
 * `x-request-id` header forwarded verbatim to `checkInVisitorForCommunity`.
 */
...
import { visitorsCheckinContract } from './contract';

export const PATCH = withErrorHandler(
  runRoute(visitorsCheckinContract, async ({ params, body, req }) => {
    ...
    return checkInVisitorForCommunity(
      communityId,
      params.id,
      actorUserId,
