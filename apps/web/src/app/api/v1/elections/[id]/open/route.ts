/**
 * Elections — open an election (state transition)
 *
 * POST /api/v1/elections/[id]/open
 * Body: { communityId }
 *
 * Plan A1 drain #42. Migrated to `runRoute(contract, handler)`; see
 ...
 *
 * `x-request-id` header forwarded verbatim to `openElectionForCommunity`.
 */
...
import { electionsOpenContract } from './contract';

export const POST = withErrorHandler(
  runRoute(electionsOpenContract, async ({ params, body, req }) => {
    ...
    return openElectionForCommunity(
      communityId,
      params.id,
      actorUserId,
